import { GameState, ClientMessageSchema } from "last-choice-shared";
...
export class GameRoom {
    ...
    roundStartedAt = null;
    roundEndsAt = null;
    consecutiveWipeCount = 0;
    winnerId = null;
    roundResults = [];
    finalDuelResults = [];
    scheduledTasks = [];
    sessions = {};
    localAlarmTimeout = null;
    ...
    async loadFromStorage() {
        ...
        this.hostPlayerId = (await this.state.storage.get("hostPlayerId")) || "";
        this.gameState = (await this.state.storage.get("gameState")) || GameState.LOBBY;
        this.currentRound = (await this.state.storage.get("currentRound")) || 0;
        this.selectableSlotCount = (await this.state.storage.get("selectableSlotCount")) || 0;
        this.selections = (await this.state.storage.get("selections")) || {};
        this.finalSelections = (await this.state.storage.get("finalSelections")) || {};
        this.roundStartedAt = (await this.state.storage.get("roundStartedAt")) || null;
        this.roundEndsAt = (await this.state.storage.get("roundEndsAt")) || null;
        this.consecutiveWipeCount = (await this.state.storage.get("consecutiveWipeCount")) || 0;
        this.winnerId = (await this.state.storage.get("winnerId")) || null;
        this.roundResults = (await this.state.storage.get("roundResults")) || [];
        this.finalDuelResults = (await this.state.storage.get("finalDuelResults")) || [];
        this.scheduledTasks = (await this.state.storage.get("scheduledTasks")) || [];
    ...
    async handleAnonymousAction(msg, ws) {
    ...
    async handleUserAction(playerId, msg, ws) {
    ...
    scheduleTask(type, executeAt, data) {
    ...
    cancelTaskByPlayerId(type, playerId) {
    ...
    async executeTask(task) {
        switch (task.type) {
            ...
            case "ROUND_TIMEOUT": {
                ...
                const { nextPlayers, result, nextState, nextSlotCount, nextConsecutiveWipeCount, winnerId } = GameEngine.processRound(this.players, this.selections, this.selectableSlotCount, this.consecutiveWipeCount, this.currentRound);
            ...
            case "REVEAL_COMPLETE": {
                if (this.gameState !== GameState.REVEALING)
                    return;
            ...
            case "FINAL_DUEL_TIMEOUT": {
                ...
                const { nextPlayers, result, nextState, winnerId } = GameEngine.processFinalDuelRound(this.players, this.finalSelections, this.currentRound);
