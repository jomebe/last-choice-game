...
export class GameEngine {
    // 일반 라운드 판정
    static processRound(players, selections, // playerId -> slot (1-indexed) or null
    slotCount, consecutiveWipeCount, roundNumber) {
        const alivePlayers = players.filter(p => p.isAlive);
        const alivePlayerIds = new Set(alivePlayers.map(p => p.id));
        // 칸별 선택 집계
        const slotChosenCount = {};
        const slotChosenBy = {};
        // 1부터 slotCount까지 초기화
        ...
        const nextPlayersMap = new Map(players.map(p => [p.id, { ...p }]));
    ...
    // 가위바위보 결과 판정 순수 함수
    static resolveFinalDuel(first, second) {
        if (first === second)
            return "DRAW";
        if (first === "ROCK") {
    ...
    // 결승전 라운드 판정
    static processFinalDuelRound(players, finalSelections, // playerId -> Choice or null
    finalRoundNumber) {
